...
def createAllPipeSections():
    ...
    #close the file
    file.close()
    print("All pipe sections: ")
    print(allPipeSections)
...
def headLossCalc(p):
    ...
    if p.getReversed(): qVal = (-1.0)*qVal
...
def dQCalc(loop):
    ...
    for section in loop:
        for sectionObj in allPipeSections:
            if (sectionObj.getPipePoints()[0].getName() in section) and (sectionObj.getPipePoints()[1].getName() in section):
                ...
                qVal = qList[len(qList)-1]
                #if (sectionObj.getReversed() == True): qVal = qVal*(-1.0)
                #print("sumNumerator += " + str(k) + "*" + str(qVal) + "*" + str(abs(qVal)))
                #print("sumNumerator += " + str((k*qVal*abs(qVal))))
    ...
    #print("sumNumerator: " + str(sumNumerator) + " | sumDenominator: " + str(sumDenominator))
    return (-1.0*(sumNumerator/(2*sumDenominator)))
    #currently, calls of this function return dQ = 0.0 when it should not because sumNumerator keeps getting things added and then subtracted from it, and it ends up equaling zero
...
def hardyCrossCalc():
    ...
    for loop in loopsArr:
        ...
        while ((abs(hL) > 0.01) or (abs(dQ) > 0.001)):
            ...
            for section in loop:
                ...
                for sectionObj in allPipeSections:
                    if (sectionObj.getPipePoints()[0].getName() in section) and (sectionObj.getPipePoints()[1].getName() in section):
                        ...
                        print("head loss: " + str(hL))
                        sectionObj.appendHeadLossHistory(hL)
                        print("Q: " + str(sectionObj.getRecentQVal()))
